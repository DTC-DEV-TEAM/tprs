<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    <label class='control-label col-sm-2'>{{$form['label']}}
        ...
        ?>
        <div class="{{$col_width?:'col-sm-10'}}" id="cblist5">
        <div data-val='DEFAULT-STORE' class='checkbox {{ $disabled }}' id="cblist6">
            <label for="cb0">
               <input type='checkbox' {{ $disabled }} {{ $checked }} name='{{$name}}[]' value='' id="cb1">
               TST-DEFAULT DEPARTMENT                               
            </label>
        </div>
        </div>
        @push('bottom')
        <script type="text/javascript">
            $(function() {          
                $('#approver_department_id, input:radio[name={{$parent}}]').change(function() {
                    ...
                    if(fk_value!='') {
                       // alert(fk_name);
                        //$current.html("");
                        $.get("{{CRUDBooster::mainpath('data-table')}}?table="+table+"&label="+label+"&fk_name="+fk_name+"&fk_value_new="+fk_value+"&datatable_where="+encodeURI(datatableWhere),function(response) {
                            if(response) {
                                //$current.html("");
                                var container = $('#cblist5');
                                container.empty();
                                $.each(response,function(i,obj) {

                                   // alert(obj.select_label);
