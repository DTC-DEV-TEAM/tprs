    ...
    <div class="budget_block">
        ...
        <div class="budget">
            <div class="budget_description">
                ...
                <input class="input_description" type="text" name="description[]"
            ...
            <div class="budget_description" id="step3_budget_justification">
                ...
                <div class="upload_img_parent">
                    <input type="file" name="budget_justification[]" accept="image/png, image/gif, image/jpeg"
                        id="upload_img" multiple required>
                </div>
            </div>
        </div>
    </div>
...
<div class='panel panel-default'>
    ...
    <div class="typewriter" style="display: none;">
        <div class="typewriter_middle">
            ...
            <h4 style="position: absolute; left: -58px; width: 260px; color: white; font-weight: bold;">TRANSACTION IS
                ON PROCESS...</h4>
        </div>
    </div>
    <form method="POST" action="{{ CRUDBooster::mainpath('edit-save/' . $row->id) }}" enctype="multipart/form-data"
        id="receipts_validation">
                    ...
                    <div class="budget_content">
                        <div class="budget_block">
                            <div class="budget">
                                <div class="budget_description">
                                    <label for="">Description</label>
                                    <input class="input_description" type="text" value="UNUSED AMOUNT"
                                        name="description[]" style="background-color: #eeeeee;" readonly>
                                </div>
                                <div class="budget_description">
                                    <label for="">Concept</label>
                                    <select class="js-example-basic-single brand" id="brand" name="brand[]"
                                        disabled>
                                        <option value="" selected disabled>Select Concept</option>
                                        @foreach ($brands as $brand)
                                            <option {{ $brand->id == 31 ? 'selected' : '' }}
                                ...
                                <div class="budget_description">
                                    ...
                                    <select class="js-example-basic-single location" name="location[]" disabled>
                                        <option value="" selected disabled>Select Category</option>
                                        @foreach ($locations as $location)
                                            <option {{ $location->id == 47 ? 'selected' : '' }}
                                                value="{{ $location->id }}">{{ $location->store_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="budget_description">
                                ...
                                <div class="budget_description">
                                    ...
                                    <input type="number" name="amount[]" min="0" step="0.01"
                                        class="budget_amount" value="0" id="cash_in_bank_total_value">
                                </div>
                                <div class="budget_description" id="step3_budget_justification">
                                    <label for="">Receipts</label>
                                    <div class="upload_img_parent">
                                        <input type="file" name="budget_justification[]"
                                            accept="image/png, image/gif, image/jpeg" id="upload_img"
                                            class="receipts_upload_img" multiple>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <input style="border: none;" value="{{ date('Y-m-d', strtotime($row->check_date)) }}" readonly>
                        ...
                        <div class="receipts_amount_contents">
                            <div class="total_amount_content receipts_total_amount">
                                ...
                                <input class="read_only" type="number" id="requested_amount"
                                    value="{{ $row->requested_amount }}" readonly>
                            </div>
                            <div class="total_amount_content receipts_total_amount">
                                <label for="">Used Amount:</label>
                                <input class="read_only" type="number" id="total_amount" value="0"
                                    name="total_amount" readonly>
                            </div>
                            <div class="total_amount_content receipts_total_amount">
                                <label for="">Unused Amount:</label>
                                <input class="read_only" type="number" id="unused_amount" value="0"
                                    name="unused_amount" readonly>
                            </div>
                            <div class="total_amount_content receipts_total_amount">
                                <label for="">Remaining Balance:</label>
                                <input class="read_only" type="number" id="remaining_balance" value="0"
                                    name="remaining_blance" readonly>
                            </div>
                        </div>
                    </div>
                    ...
                    <br>
                    <div>
                        <span style="font-size: 15px;"><span style="color: red; font-weight: bold;">Notes:
                            </span>"After submitting the form", you can now return the receipts and unused balance along
                            with your reference number."</span>
                        <br>
                        <span style="font-size: 15px;"><span
                                style="color: rgb(255, 255, 255); font-weight: bold; visibility: hidden;">Notes:
                            </span>"For uploading of multiple receipts press <span style="color: green;">ctrl left
                                click to the image</span>. <span style="color: green;">Accepts Image
                                only</span>"</span>
                    </div>
                </div>
            </div>
