...
class AdminPendingPaymentRequestController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		if (CRUDBooster::isUpdate()) {
			if (CRUDBooster::myPrivilegeName() == "Approver") {
				...
				$Transmitted = RequestStatus::where('id', 11)->value('id');
				$Paid = RequestStatus::where('id', 4)->value('id');

				$this->addaction[] = ['title' => 'Edit', 'url' => CRUDBooster::mainpath('getRequestPayment/[id]'), 'icon' => 'fa fa-pencil', "showIf" => "[status_id] == $Transmitted"];
				...
				$this->addaction[] = ['title' => 'Edit', 'url' => CRUDBooster::mainpath('getRequestConfirmation/[id]'), 'icon' => 'fa fa-pencil', "showIf" => "[status_id] == $Printed"];
			}
	...
	public function hook_query_index(&$query)
	{
		...
		if (CRUDBooster::myPrivilegeName() == "Approver") {
			...
			$query->where(function ($sub_query) {
				...
				$sub_query->where('prf_header.status_id', $Transmitted)->orderBy('prf_header.status_id', 'DESC')->orderBy('prf_header.id', 'ASC');
	...
	public function hook_before_edit(&$postdata, $id)
	{
		...
		if (CRUDBooster::myPrivilegeName() == "Approver") {
			...
			$ap_checker_comments = $fields['ap_checker_comments'];
			...
			$postdata['ap_checker_comments'] = $ap_checker_comments;

			$invoice_number = $fields['invoice_number'];
			$invoice_date = $fields['invoice_date'];
			$item_id = $fields['item_id'];
			$new_item_id = $fields['new_item_id'];
			...
			for ($x = 0; $x < count((array) $item_id); $x++) {
				...
				PRFBody::where('id', $item_id[$x])
					->update([
						'invoice_number' => $invoice_number[$x],
			...
			$flag = count((array) $item_id);

			if ($new_item_id != null) {

				for ($item = 0; $item < count((array) $new_item_id); $item++) {
					PRFBody::create([
						'prf_header_id' => $new_item_id[$item],
						'invoice_number' => $invoice_number[$flag + $item],
						'invoice_date' => $invoice_date[$flag + $item],
						'invoice_type_id' => $invoice_type_id[$flag + $item],
						'payment_status_id' => $payment_status_id[$flag + $item],
						'vat_type_id' => $vat_type_id[$flag + $item],
						'product_id' => $product_id[$flag + $item],

						'account_id' => $account_id[$flag + $item],
						'brand_id' => $brand_id[$flag + $item],
						'category_id' => $category_id[$flag + $item],
						'particulars' => $particulars[$flag + $item],
						'currency_id' => $currency_id[$flag + $item],
						'quantity' => $quantity[$flag + $item],
						'line_value' => $line_value[$flag + $item],
						'total_value' => $total_value[$flag + $item],
						'location_id' => $location_id[$flag + $item]
					]);
				}
			...
			if ($prf_header->status_id == 4) {
				...
				$paid_date = $fields['date_release'];
				$check_number = $fields['check_number'];

				$postdata['paid_date'] = $paid_date;
				$postdata['check_number'] = $check_number;
			...
			$transmittal_date = $fields['transmittal_date'];

			//update ap supervisor columns
			$postdata['ap_supervisor'] = CRUDBooster::myId();
			$postdata['status_id'] = RequestStatus::where('id', 11)->value('id');
			$postdata['supervisor_approval_date'] = date('Y-m-d');
			$postdata['transmittal_date'] = $transmittal_date;
		}
	...
	public function hook_after_edit($id)
	{
		...
		if (CRUDBooster::myPrivilegeName() == "Approver") {
			...
			CRUDBooster::redirect(CRUDBooster::mainpath(), trans("crudbooster.alert_petty_cash_confirm_success", ['reference_number' => $prf_header->reference_number]), 'info');
		}
	}
	...
	public function getRequestConfirmation($id)
	{
		...
		$data['Header'] = PRFHeader::
			...
			->select(
				...
				'company.interco_name as intercolevel'
			)
			->where('prf_header.id', $id)->first();
		...
		return view("prf.transmittal", $data);
	}
	...
	public function getRequestPayment($id)
	{
		...
		$data['Header'] = PRFHeader::
			...
			->leftjoin('cms_users as validator', 'prf_header.validated_by', '=', 'validator.id')
			->leftjoin('cms_users as printed', 'prf_header.printed_by', '=', 'printed.id')
			->leftjoin('cms_users as transmitted', 'prf_header.ap_supervisor', '=', 'transmitted.id')
			->leftjoin('interco as company', 'prf_header.interco_id', '=', 'company.id')
			->select(
				'department.*',
				'customer.*',
				...
				'prf_header.id as requested_id',
				'printed.name as printedlevel',
				'transmitted.name as transmittedlevel',
