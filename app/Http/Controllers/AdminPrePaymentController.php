...
class AdminPrePaymentController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		if ((CRUDBooster::getCurrentMethod() == 'getIndex') && (CRUDBooster::myPrivilegeName() != 'Treasury') && (CRUDBooster::myPrivilegeName() != 'AP Checker') && (CRUDBooster::myPrivilegeName() != 'AP Supervisor')) {
	...
	public function hook_before_edit(&$postdata, $id)
	{
		...
		if ($status == 7) {
			...
			if ($submit_btn == 'Save') {
				...
				$postdata['ap_checker_date_recorded'] = $return_inputs['ap_checker_date_recorded'];
		...
		if ($status == 3) {
			...
			for ($i = 0; $i < count($description); $i++) {
				...
				$insert_pre_payment_body[] = [
					...
					'brand' => $i === 0 ? null : $brand[$i - 1],
					'location' => $i === 0 ? null : $location[$i - 1],
					'category' => $category[$i],
					'account' => $account[$i],
					'currency' => $currency[$i],
					'amount' => $amount[$i],
		...
		if ($status == 4) {
			...
			$isBrandNotEqualSize = is_array($brand) && count($brand) !== count($description);
			$isLocationNotEqualSize = is_array($location) && count($location) !== count($description);

			for ($i = 0; $i < count((array) $description); $i++) {
				$requested_project[] = [
					'pre_payment_id' => $pre_payment_id[$i],
					'description' => $description[$i],
					'brand' => ($isBrandNotEqualSize && $i === 0) ? null : ($isBrandNotEqualSize ? $brand[$i - 1] : $brand[$i]),
					'location' => ($isLocationNotEqualSize && $i === 0) ? null : ($isLocationNotEqualSize ? $location[$i - 1] : $location[$i]),
					'category' => $category[$i],
					'account' => $account[$i],
					'currency' => $currency[$i],
					'qty' => $qty[$i],
			...
			// Insert budget information
			for ($i = 0; $i < count((array) $requested_id); $i++) {
				DB::table('pre_payment_body')->updateOrInsert(
					['id' => $requested_id[$i]],
					$requested_project[$i]
				);
			}
	...
	public function add_request(Request $request)
	{
		...
		$id = DB::table('pre_payment')->insertGetId([
