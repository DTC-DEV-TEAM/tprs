<?php namespace App\Http\Controllers;
	...
	class AdminApprovalMatricesController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->col[] = ["label"=>"Sub Department","name"=>"sub_department_id"];
			// $this->col[] = ["label"=>"Channel","name"=>"channels_id","join"=>"channels,channel_name"];
			$this->col[] = ["label"=>"Location","name"=>"store_list"];
			...
			$this->form[] = ['label'=>'Privilege Name','name'=>'id_cms_privileges','type'=>'select','validation'=>'required|integer|min:0','width'=>'col-sm-5','datatable'=>'cms_privileges,name','datatable_where'=>"name LIKE '%REQUESTOR'  ||  name LIKE '%Approver'  ||  name LIKE '%Custodian' "];
			...
			$this->form[] = ['label'=>'Department','name'=>'department_id','type'=>'select','validation'=>'required|integer|min:0','width'=>'col-sm-5','datatable'=>'department,department_name'];
			...
			$this->form[] = ['label'=>'Sub Department','name'=>'sub_department_id','type'=>'check-box','validation'=>'required','width'=>'col-sm-5','datatable'=>'sub_department,sub_department_name','relationship_table'=>'sub_department','parent_select'=>'department_id'];
			...
			$this->form[] = ['label'=>'Location','name'=>'store_list','type'=>'check-box-1','validation'=>'required','width'=>'col-sm-5','datatable'=>'stores,store_name','relationship_table'=>'stores','parent_select'=>'channels_id'];
	    ...
	    */    
	    public function hook_row_index($column_index,&$column_value) {	        
			...
			if($column_index == 4){//column order
				$depLists = $this->depListing($column_value);
				
				foreach ($depLists as $value) {
					$col_values .= '<span stye="display: block;" class="label label-primary">'.$value.'</span><br>';
				}
				$column_value = $col_values;
			}
	    ...
	    */
	    public function hook_before_add(&$postdata) {        
			...
			foreach ($departmentArray as $key => $value) {
				$departmentData[$key] = preg_replace("/[^0-9]/","",$value);
			}
			...
			$postdata['sub_department_id'] = implode(",", $departmentData);
...
// 			}
	    ...
	    */
		...
		public function depListing($depids) {
			
			$departments = explode(",", $depids);
			return DB::table('sub_department')->whereIn('id', $departments)->pluck('sub_department_name');
		}
