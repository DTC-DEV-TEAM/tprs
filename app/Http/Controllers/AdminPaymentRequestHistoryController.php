...
class AdminPaymentRequestHistoryController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		if (CRUDBooster::getCurrentMethod() == 'getIndex') {
			$this->index_button[] = [
				...
				"label" => "Export",
				"icon" => "fa fa-download",
				"url" => CRUDBooster::adminpath('export-prf') . '?' . urldecode(http_build_query(@$_GET))
			];
	...
	public function hook_row_index($column_index, &$column_value)
	{
		...
		if ($column_index == 2) {
			...
			} else if ($column_value == $Trasnsmitted) {
				$column_value = '<span style="background-color: #8a2be2; color: white;" class="label">' . $Trasnsmitted . '</span>';
