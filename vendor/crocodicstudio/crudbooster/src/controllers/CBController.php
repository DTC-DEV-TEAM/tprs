...
class CBController extends Controller
{
    ...
    public function getDataTable()
    {
        ...
        $foreign_key_value_new = Request::get('fk_value_new');

        if (!empty($foreign_key_value_new )) {

            if ($table && $label && $foreign_key_name && $foreign_key_value_new) {
                $query = DB::table($table);
                if ($datatableWhere) {
                    $query->whereRaw($datatableWhere);
                }
                $query->select('id as select_value', $label.' as select_label');
                $query->wherein($foreign_key_name, explode(",",$foreign_key_value_new));
